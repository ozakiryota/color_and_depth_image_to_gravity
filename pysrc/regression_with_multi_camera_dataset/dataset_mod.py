...
class OriginalDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        camera_angle = self.data_list[index][5]
        acc_list = self.data_list[index][:3]
        ## to numpy
        depth_img_numpy = np.load(depth_img_path)
        color_img_pil = Image.open(color_img_path)
        acc_numpy = np.array(acc_list)
        ## tansform
        color_img_trans, depth_img_trans, acc_trans = self.transform(color_img_pil, depth_img_numpy, acc_numpy, camera_angle, phase=self.phase)
