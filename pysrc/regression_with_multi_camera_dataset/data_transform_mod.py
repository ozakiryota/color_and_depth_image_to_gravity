...
class DataTransform():
    ...
    def __call__(self, color_img_pil, depth_img_numpy, acc_numpy, heading_angle, phase="train"):
        ## slide
        depth_img_numpy = self.slidePixels(depth_img_numpy, heading_angle)
        ## augemntation
        if phase == "train":
            ## mirror
    ...
    def slidePixels(self, depth_img_numpy, heading_angle):
        ## slide: left -> right (rotate: CCW along Z-axis)
        slide_pixel = int(heading_angle / (2*math.pi) * depth_img_numpy.shape[1])
        slid_depth_img_numpy = np.roll(depth_img_numpy, slide_pixel, axis=1)
        return slid_depth_img_numpy

##### test #####
# ## color image
# color_img_path = "../../../dataset_image_to_gravity/AirSim/example/camera_180.jpg"
# color_img_pil = Image.open(color_img_path)
# camera_angle = 180.0 / 180.0 * math.pi 
# ## depth image
...
# color_img_trans, depth_img_trans, acc_trans = transform(color_img_pil, depth_img_numpy, acc_numpy, camera_angle, phase="train")
